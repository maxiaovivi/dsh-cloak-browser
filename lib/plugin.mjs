...
function buildCloakLaunchOptions(config, env = process.env) {
  const proxy = config.proxyEnv ? env[config.proxyEnv] : undefined;
  const args = [];
  if (config.fingerprintSeed) args.push(`--fingerprint=${config.fingerprintSeed}`);
...
class SessionMap {
  ...
  async create(agent) {
    ...
    const options = buildCloakLaunchOptions(this.config);
    let context;
...
function toolDefinitions(ctx, sessions, config) {
  ...
  return [
    ...
    defineTool({
      ...
      output: jsonOutput(),
      timeoutMs: config.typingTimeoutMs,
      async execute(args, exec) {
        return run(exec, async (session) => {
          const { locator } = await session.locatorForRef(args.ref);
          await locator.scrollIntoViewIfNeeded();
          await locator.fill(args.text, { timeout: config.typingTimeoutMs });
...
export { Config, SessionMap, apply, buildCloakLaunchOptions, createCloakBrowserPlugin, inject, name, normalizeConfig };
