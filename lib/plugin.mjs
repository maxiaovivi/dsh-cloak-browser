...
function toolDefinitions(ctx, sessions, config) {
  ...
  return [
    defineTool({
      name: "browser_open",
      description: "Open this Agent's isolated CloakBrowser session. When a URL is provided, the result automatically includes a fresh page snapshot and usable element refs.",
...
- Prefer browser_open with a URL or browser_navigate; their result already contains a snapshot and usable refs.
...
export { Config, SessionMap, apply, buildCloakLaunchOptions, createCloakBrowserPlugin, fingerprintSeedForAgent, inject, name, normalizeConfig };
