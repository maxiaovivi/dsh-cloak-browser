...
class SessionMap {
  ...
  async get(agent, signal) {
    ...
    if (!pending) {
      ...
      if (license.tier === "free" && [...this.sessions.keys()].some((owner) => owner !== agent)) {
        throw new Error("Another Agent in this DSH process already owns the single CloakBrowser Free session; start with browser_open to receive a stable occupancy result.");
      }
  ...
  async open(agent, signal, task) {
    ...
    return this.serializeLaunch(async () => {
      ...
      try {
        return await this.run(agent, signal, task);
      } catch (error) {
...
function toolDefinitions(ctx, sessions, config) {
  ...
  return [
    defineTool({
      name: "browser_open",
      description: "Open this Agent's isolated CloakBrowser session. Validated Free keys default to an available single session; local launches are serialized and occupancy is reported without repeated retries. When a URL is provided, the result automatically includes a fresh page snapshot and usable element refs.",
      parameters: { url: { type: "string", description: "Optional absolute http(s) URL." } },
      output: jsonOutput(),
      timeoutMs,
      async execute(args, exec) {
        return sessions.open(exec.agent, exec.signal, async (session) => {
          const page = await session.page();
          if (args.url) {
...
- Start a fresh browser workflow with browser_open. Validated Free keys default to an available session and launch without asking the user. If browser_open returns status=not_started because the local process or license server reports an occupied seat, do not retry in a loop; continue with a lighter web tool when possible or explain that the browser seat is busy.
