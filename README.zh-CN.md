...
当环境变量或本地保存的 License 被验证为 **Free plan** 时，插件默认席位无人使用，直接启动，
不会询问用户。插件会串行化同一 DSH 进程内的 Free 会话并发启动，并在本地或 License Server
已占用时返回明确状态，不再循环重试。插件无法撤销另一台机器或异常退出旧进程持有的服务器端
租约；这类租约仍需由上游关闭或等待过期。
...
- 自动识别 License 套餐：验证为 Free 的 Key 无需询问用户即可启动；同一 DSH 进程内的 Free
  会话并发启动会被串行化。
...
```text
browser_open(url)
  → Free、付费套餐或无 Key 本地构建：无需询问，直接启动
  → Free 席位已占用：返回 not_started，不循环重试
  → 返回 snapshot + refs
...
| Tool | 功能 |
|---|---|
| `browser_open` | 无需用户确认，延迟打开会话；传入 URL 时同时返回 Snapshot |
